...
pub struct Settings {
    ...
    /// The name of the `minifix` crate for imports. `minifix` by default.
    pub minifix_crate_name: String,
...
impl Default for Settings {
    fn default() -> Self {
        Self {
            ...
            minifix_crate_name: "minifix".to_string(),
        }
    }
}
...
pub fn gen_definitions(
    ...
    let code = formatdoc!(
        r#"
            ...
            use {minifix_path}::dict::{{FieldLocation, FixDatatype}};
            use {minifix_path}::definitions::HardCodedFixFieldDefinition;
            use {minifix_path}::FieldType;
        ...
        minifix_path = settings.minifix_crate_name,
    );
    code
}
