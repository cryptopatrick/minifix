...
pub fn derive_fix_value(input: TokenStream) -> TokenStream {
    ...
    let darling_context =
        DataFieldWithVariants::from_derive_input(&ast).unwrap();
    ...
    let deserialize_matching_cases = darling_context
        ...
        .map_enum_variants(|enum_variant| {
            let enum_discriminant = enum_variant.variant.as_str();
            let enum_variant = enum_variant.ident;
            let bstring: proc_macro2::TokenStream = TokenTree::from(
                Literal::byte_string(enum_discriminant.as_bytes()),
            )
            .into();
            quote! {
        ...
        .take_enum()
        .expect("Invalid enum");
    let minifix_crate_info = proc_macro_crate::crate_name("minifix")
        .expect("Cargo.toml minifix issues");
    let minifix_crate_name = match minifix_crate_info {
        proc_macro_crate::FoundCrate::Itself => {
            Ident::new("crate", Span::call_site())
        }
        proc_macro_crate::FoundCrate::Name(s) => {
