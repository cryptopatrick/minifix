...
fn decode_example() {
    ...
    println!("\n📋 Header Fields:");
    println!("  BeginString: {:?}", msg.get::<&[u8]>(fix42::BEGIN_STRING).unwrap());
    println!("  MsgType: {:?}", msg.get::<&[u8]>(fix42::MSG_TYPE).unwrap());
    println!("  SenderCompID: {:?}", msg.get::<&[u8]>(fix42::SENDER_COMP_ID).unwrap());
    println!("  TargetCompID: {:?}", msg.get::<&[u8]>(fix42::TARGET_COMP_ID).unwrap());
    ...
    for (i, entry) in md_entries.entries().enumerate() {
        println!("\n  Entry {}:", i + 1);
        println!("    MDUpdateAction: {:?}", entry.get::<&[u8]>(fix42::MD_UPDATE_ACTION).unwrap());
        println!("    MDEntryType: {:?}", entry.get::<&[u8]>(fix42::MD_ENTRY_TYPE).unwrap());
        println!("    Symbol: {:?}", entry.get::<&[u8]>(fix42::SYMBOL).unwrap());
        println!("    MDEntryPx: {:?}", entry.get::<&[u8]>(fix42::MD_ENTRY_PX).unwrap());
        println!("    Currency: {:?}", entry.get::<&[u8]>(fix42::CURRENCY).unwrap());
        println!("    MDEntrySize: {:?}", entry.get::<u32>(fix42::MD_ENTRY_SIZE).unwrap());
    }
    
    // Demonstrate different ways to access the same field
    println!("\n🎯 Field Access Examples:");
    println!("  Using field constant: {:?}", msg.get::<&[u8]>(fix42::SENDER_COMP_ID).unwrap());
...
fn encode_example() {
    ...
    let display_msg = String::from_utf8_lossy(encoded_msg)
        .replace('\x01', "|");
    println!("  {}", display_msg);
    ...
    let decoded_msg = decoder
        .decode(encoded_msg)
        .expect("Failed to decode our own message");
    
    println!("  Original ClOrdID: {:?}", decoded_msg.get::<&[u8]>(fix43::CL_ORD_ID).unwrap());
    println!("  Original Symbol: {:?}", decoded_msg.get::<&[u8]>(fix43::SYMBOL).unwrap());
    println!("  Original Side: {:?}", decoded_msg.get::<&[u8]>(fix43::SIDE).unwrap());
    println!("  Original OrderQty: {:?}", decoded_msg.get::<u32>(fix43::ORDER_QTY).unwrap());
...
mod tests {
    ...
    fn test_encode_example() {
        ...
        assert_eq!(decoded_msg.get::<&[u8]>(fix43::CL_ORD_ID).unwrap(), b"TEST-ORDER");
