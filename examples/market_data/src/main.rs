...
fn process_market_data_snapshot() {
    ...
    let msg = decoder.decode(MARKET_DATA_SNAPSHOT)
        .expect("Failed to decode market data snapshot");
    
    // Extract header information
    println!("📊 Market Data Snapshot:");
    println!("  Source: {:?}", msg.get::<&[u8]>(fix44::SENDER_COMP_ID).unwrap());
    println!("  Request ID: {:?}", msg.get::<&[u8]>(fix44::MD_REQ_ID).unwrap());
...
fn process_market_data_incremental() {
    ...
    let msg = decoder.decode(MARKET_DATA_INCREMENTAL)
...
fn process_multi_currency_snapshot() {
    ...
    let msg = decoder.decode(MARKET_DATA_MULTI_CCY)
    ...
    for entry in entries.entries() {
        let symbol = String::from_utf8_lossy(entry.get::<&[u8]>(fix44::SYMBOL).unwrap()).to_string();
        ...
        let price_str = std::str::from_utf8(entry.get::<&[u8]>(fix44::MD_ENTRY_PX).unwrap()).unwrap();
        let price: f64 = price_str.parse().unwrap();
        let size: u64 = entry.get::<u32>(fix44::MD_ENTRY_SIZE).unwrap() as u64;
        let currency = String::from_utf8_lossy(entry.get::<&[u8]>(fix44::CURRENCY).unwrap()).to_string();
        
        let level = PriceLevel { side, price, size, currency };
        
        books.entry(symbol.clone())
...
fn market_data_subscription_workflow() {
    ...
    let display_msg = String::from_utf8_lossy(request_msg).replace('\x01', "|");
    println!("  {}", display_msg);
    ...
    let display_reject = String::from_utf8_lossy(reject_msg).replace('\x01', "|");
    println!("  {}", display_reject);
    
    println!("\n✅ Subscription workflow demonstration complete");
}

fn process_md_entries(msg: &Message<&[u8]>, msg_type: &str) -> Result<MarketDataBook, Box<dyn std::error::Error>> {
    let entries = msg.group(fix44::NO_MD_ENTRIES)?;
    ...
    for (i, entry) in entries.entries().enumerate() {
        ...
        if symbol.is_empty() {
            symbol = String::from_utf8_lossy(entry.get::<&[u8]>(fix44::SYMBOL)?).to_string();
        }
        
        let side_byte = entry.get::<&[u8]>(fix44::MD_ENTRY_TYPE)?;
        let side = match side_byte {
            b"0" => "BID",
            b"1" => "OFFER",
            _ => "UNKNOWN",
        ...
        let price_str = std::str::from_utf8(entry.get::<&[u8]>(fix44::MD_ENTRY_PX)?)?;
        let price: f64 = price_str.parse()?;
        let size: u64 = entry.get::<u32>(fix44::MD_ENTRY_SIZE)? as u64;
        let currency = String::from_utf8_lossy(entry.get::<&[u8]>(fix44::CURRENCY)?).to_string();
        
        levels.push(PriceLevel { side, price, size, currency });
        
        println!("    Entry {}: {} @ {} size {} {}",
            i + 1, 
            match side_byte { b"0" => "BID", b"1" => "OFFER", _ => "?" },
            price_str, 
            size,
            currency
        );
...
fn print_order_book(book: &MarketDataBook) {
    let bids: Vec<_> = book.levels.iter().filter(|l| l.side == "BID").collect();
    let offers: Vec<_> = book.levels.iter().filter(|l| l.side == "OFFER").collect();
    
    println!("    {'':>12} | {:^8} | {:^12} | {:^8} | {:<12}",
    ...
    for i in 0..max_levels {
        let bid_info = bids.get(i).map(|b| (b.size, b.price, &b.currency))
            .unwrap_or((0, 0.0, &String::new()));
        let offer_info = offers.get(i).map(|o| (o.size, o.price, &o.currency))
            .unwrap_or((0, 0.0, &String::new()));
            
        let symbol_display = if i == max_levels / 2 { &book.symbol } else { "" };
        
        println!("    {:>12} | {:>8.5} | {:^12} | {:<8.5} | {:<12}",
            if bid_info.0 > 0 { format!("{} {}", bid_info.0, bid_info.2) } else { String::new() },
            if bid_info.1 > 0.0 { format!("{:.5}", bid_info.1) } else { String::new() },
            symbol_display,
            if offer_info.1 > 0.0 { format!("{:.5}", offer_info.1) } else { String::new() },
            if offer_info.0 > 0 { format!("{} {}", offer_info.0, offer_info.2) } else { String::new() }
        );
    }
}
...
mod tests {
    ...
    fn test_market_data_parsing() {
        ...
        assert_eq!(msg.get::<&[u8]>(fix44::SENDER_COMP_ID).unwrap(), b"BLOOMBERG");
        ...
        assert_eq!(first_entry.get::<&[u8]>(fix44::SYMBOL).unwrap(), b"EUR/USD");
        assert_eq!(first_entry.get::<&[u8]>(fix44::MD_ENTRY_TYPE).unwrap(), b"0"); // BID
    }
    ...
    fn test_multi_currency_parsing() {
        ...
        for entry in entries.entries() {
            ...
            symbols.insert(std::str::from_utf8(symbol).unwrap());
        }
